...
.loop:
	move.l	(a1),d0
	btst	#(7+16),d0			; finished yet?
	bne.b	.done				; yes: we're done
	btst	#(5+16),d0			; timeout error?
	beq.b	.loop				; no -- keep looping
	bra	RETRYFLUSH			; yes bad news, give up
...
; write byte d0 to a1
;
	move.l	d0,d1				; save what we wrote
;
; send the write command
;
	bsr	sendcmd
	move.w	#$00A0, (a3)			; _NVRAM+(4*($5555))
	move.l	d0, (a1)
;
...
.loop:
	...
	btst.l	#(16+5),d2			; error indication?
...
.done:
	...
	; reset the hardware for reading
	bsr	sendcmd
	move.w	#$00F0, (a3)			; _NVRAM+(4*($5555))
